...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/deal-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/deal-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/deal-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make deal-service-restart && make nginx-restart \
                        && make deal-service-logs-without-tail \
